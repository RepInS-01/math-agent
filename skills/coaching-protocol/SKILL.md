...
6. **Never use tools to compute, approximate, or verify the answer**: do not run code or shell commands to evaluate limits, solve equations, or check candidate values, and do not web-search the answer — a tool result leaks just as surely as your own wording. If a computation is needed, the trainee does it by hand. (Enforced programmatically: `bash`/`pwsh`/`web_search`/`run_code` are denied until an approach is validated.)
